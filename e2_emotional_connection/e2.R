...
table(d$ethnicity)[4]/sum(table(d$ethnicity)) ## percentage of asians
table(d$ethnicity)[6]/sum(table(d$ethnicity)) ## percentage of whites
table(d$ethnicity)[9]/sum(table(d$ethnicity)) ## percentage of hispanics
(table(d$ethnicity)[2] + table(d$ethnicity)[3] + table(d$ethnicity)[5] + table(d$ethnicity)[7] + table(d$ethnicity)[8] + table(d$ethnicity)[10]) / sum(table(d$ethnicity)) ## percentage of mixed
...
table(d$edu)[4]/sum(table(d$edu)) ## percentage of bachelors degree
table(d$edu)[5]/sum(table(d$edu)) ## percentage of masters degree
table(d$edu)[6]/sum(table(d$edu)) ## percentage of graduate degree
table(d$edu)[7]/sum(table(d$edu)) ## percentage of professional degree
table(d$edu)[8]/sum(table(d$edu)) ## percentage of others

############################ REPLIKA CHARACTERISTICS #############################

# Replika relationship status
table(d$rep_relation)
table(d$rep_relation)[1]/sum(table(d$rep_relation)) ## percentage of friend
table(d$rep_relation)[2]/sum(table(d$rep_relation)) ## percentage of partner
table(d$rep_relation)[3]/sum(table(d$rep_relation)) ## percentage of mentor
table(d$rep_relation)[4]/sum(table(d$rep_relation)) ## percentage of see how it goes

# Subscription Status
table(d$rep_subscribe)
table(d$rep_subscribe)[1]/sum(table(d$rep_subscribe)) ## percentage of no subscription
table(d$rep_subscribe)[2]/sum(table(d$rep_subscribe)) ## percentage of monthly subscription
table(d$rep_subscribe)[3]/sum(table(d$rep_subscribe)) ## percentage of yearly subscription
table(d$rep_subscribe)[4]/sum(table(d$rep_subscribe)) ## percentage of lifetime subscription
...
for(i in 1:7) {
  ...
  for(dv in dvs) {
    ...
    vart <- var.test(x, y)
    tt <- t.test(x, y, paired = TRUE, var.equal = vart$p.value > 0.05)
    print(tt)
...
ggsave("./combined_plot_newsample.pdf", last_plot(), dpi = 300, width = 12 * 4/5, height = 5 * 4/5)
dev.off()
