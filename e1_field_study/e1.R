...
for(ctype in c('posts', 'comments')) { #, 'posts', 'comments'
  print(paste0("*-*-*-*-* ", ctype, " *-*-*-*-*"))
  ...
  for(i in 1:length(doc.list)) {
    ...
    for(j in 1:length(mourn_terms)) {
      ...
      if(exists) {
        ...
        #if(data$emotion[i] == "sadness") {
        #  # Print if it contains the word 'mourn'
        #  if(grepl('mourn', paste(doc, collapse = ' '), fixed = TRUE)) {
        #    print(data[i, 'titlencontent'])
        #  }
        #}
        break
      }
...
plot_emotion_relevant <- function(emotions_relevant, ctype, before_after, width=1080, height=1080) {
