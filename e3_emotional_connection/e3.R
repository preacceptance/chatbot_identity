...
pwr.t.test(n = NULL, d = 0.5, sig.level = 0.05, power = 0.95, type="paired")
...
for(dv in dvs) {
  ...
  df_anova <- data.frame(
    ...
    app = d[, paste0('X4_', dv, '_1')],
    colleague = d[, paste0('X5_', dv, '_1')],
    friend = d[, paste0('X6_', dv, '_1')],
    family = d[, paste0('X7_', dv, '_1')],
    replika = d[, paste0('X8_', dv, '_1')]
  )
  ...
  # Classical eta-squared with two-sided 95% CIs (effect sizes for condition)
  print(effectsize::eta_squared(anova_model, partial = FALSE, alternative = "two.sided"))

  # --- Assumption checks for the RM ANOVA + non-parametric robustness ---
  ...
  mlm <- lm(wide_mat ~ 1)
  idata <- data.frame(condition = factor(1:8))
  av <- car::Anova(mlm, idata = idata, idesign = ~condition, type = 3)
  av_s <- summary(av)
  print(paste0("Mauchly's W = ", round(av_s$sphericity.tests[1, "Test statistic"], 3),
               ", p = ", signif(av_s$sphericity.tests[1, "p-value"], 3)))
  print(paste0("Greenhouse-Geisser epsilon = ", round(av_s$pval.adjustments[1, "GG eps"], 3),
               ", GG-corrected p = ", signif(av_s$pval.adjustments[1, "Pr(>F[GG])"], 3)))
  # Friedman test (non-parametric, repeated-measures omnibus)
  ft <- friedman.test(wide_mat)
...
for (dv in dvs) {
  ...
  # Print mean and SD of replika (y)
  mean_y <- mean(d[, paste0('X8_', dv, '_1')])
  sd_y <- sd(d[, paste0('X8_', dv, '_1')])
  print(paste0("Replika; M = ", round(mean_y, 2), " (", round(sd_y, 2), ")"))
...
# Power test with friend vs. replika for inclusion (d = 0.47)
pwr.t.test(n = NULL, d = 0.47, sig.level = 0.05, power = 0.95, type="paired")

stars[stars$value == 0, 'value'] <- ""

# Overall correlation between support and satisfaction across all conditions
x_all <- c()
y_all <- c()
for (cond in conditions) {
  x_all <- c(x_all, d[, paste0('X', which(conditions == cond), '_support_1')])
  y_all <- c(y_all, d[, paste0('X', which(conditions == cond), '_satisfaction_1')])
}
cor_test_all <- cor.test(x_all, y_all, method = "pearson")
print(paste0("Overall correlation between support and satisfaction across all conditions: r = ", round(cor_test_all$estimate, 2), 
             ", p = ", round(cor_test_all$p.value, 3)))
...
plot_all_dvs <- function(df_plot, stars_df = stars) {
  # Human-readable legend labels for the three DVs (df_plot$dv is coded
  # a_/b_/c_ so the grey shades sort correctly: lightest = satisfaction).
  dv_labels <- c(a_satisfaction = "Relationship Satisfaction",
                 b_support = "Social Support",
                 c_inclusion = "Closeness")

  # Map the significance-stars data frame (short DV names, lowercase condition,
  # computed above from Bonferroni-corrected paired t-tests vs. Replika) onto the
  # plot's naming. All stars are drawn at the same height, above the dot clouds
  # (which top out at 100).
  star_pos <- stars_df
  star_pos$dv <- c(satisfaction = "a_satisfaction",
                   support = "b_support",
                   inclusion = "c_inclusion")[star_pos$dv]
  star_pos$condition <- tools::toTitleCase(star_pos$condition)
  names(star_pos)[names(star_pos) == "value"] <- "star"
  star_pos$y <- 110

  # Bold the "Replika" tick label (the reference condition) on the x axis.
  x_labs <- lapply(positions, function(b) if (identical(b, "Replika")) bquote(bold(.(b))) else b)
